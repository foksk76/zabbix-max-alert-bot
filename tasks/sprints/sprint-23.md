...
- [ ] Review с человеком перед Phase 2
...
- [ ] `npm test` — без регрессий
- [ ] Ручная проверка: `IDP_ISSUER=http://169.254.169.254` → запуск падает с ошибкой SSRF
- [ ] Ручная проверка: `IDP_REQUIRE_DISCOVERY=true` + невалидный discovery → запуск падает
- [ ] Review с человеком перед закрытием спринта

## Приёмочные критерии (Definition of Done)

- [ ] `npm test` — все тесты проходят (0 failures, цель 530+)
- [ ] Policy-тест `tests/repo-structure.test.js` зелёный (нет новых runtime-зависимостей)
- [ ] Rate limiting: 21-й запрос к `/api/auth/login` за 60с → 429 + `Retry-After`
- [ ] Concurrency cap: 6-й параллельный callback → 429
- [ ] SSRF: private/loopback/link-local IP в `IDP_ISSUER` → отказ на старте
- [ ] Обратная совместимость: `AUTH_RATE_LIMIT=false` и `IDP_REQUIRE_DISCOVERY=false` (defaults) сохраняют текущее поведение
- [ ] Новые ENV vars задокументированы в `examples/bot-platform/env.example` и `INSTALL.md`
- [ ] Safe logger: в логах rate-limit/SSRF-событий нет токенов/секретов
- [ ] SSRF-логирование: hostname + reason на warn, resolved IP — только на debug
