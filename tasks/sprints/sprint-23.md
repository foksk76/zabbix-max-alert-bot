...
**Acceptance criteria:**
- [x] `createAuthRateLimiter(options)` — factory function
- [x] `options.maxAuthRequests` (int, default `20`) — лимит запросов на окно
- [x] `options.windowMs` (int, default `60_000`) — размер sliding window (60с)
- [x] `options.maxConcurrentCallbacks` (int, default `5`) — cap in-flight callback'ов
- [x] `options.now` (injectable clock, для тестов)
- [x] `tryAcquireAuthRequest()` → `{ allowed: boolean, reason: 'rate-limit' | null }`
  (синхронно, non-blocking — для login + callback)
- [x] `tryAcquireCallback()` → `{ allowed: boolean, reason: 'concurrency' | null }`
  (синхронно — отдельный счётчик для дорогих callback'ов)
- [x] `releaseCallback()` — декремент in-flight счётчика (вызывать в `finally`)
- [x] `stats()` → `{ authRequests: N, inFlightCallbacks: N }` (для /readyz/логов)
- [x] Sliding window eviction (как в ADR-0030): `shift()` timestamp'ов старше windowMs

**Verification:**
- [x] `node --test tests/queue-monitor/api/auth-rate-limit.test.js` — тесты проходят
- [x] Тесты покрывают: window eviction, лимит срабатывает на N+1, concurrency
...
**Acceptance criteria:**
- [x] `createAuthRoutes(options)` принимает опциональный `options.rateLimiter`
- [x] Если `rateLimiter` не передан — auth-routes работают как раньше (обратная совместимость, bearer-only / test scenarios)
- [x] `login()`: вызывает `tryAcquireAuthRequest()`; при `allowed: false` →
      `{ statusCode: 429, headers: { 'Retry-After': '60' }, body: { error: 'Too Many Requests' } }`
- [x] `callback()`: вызывает `tryAcquireAuthRequest()` И `tryAcquireCallback()`;
      при отказе → 429; **обязательно** `releaseCallback()` в `finally` (даже при ошибке IdP)
- [x] `logout()` и `session()` — без rate limit (дёшево, session уже authenticated)
- [x] 429 не логирует токены (только ключ/throttle reason — ADR-0013)

**Verification:**
- [x] `node --test tests/queue-monitor/auth-routes.test.js` — тесты проходят
- [x] Новые тесты: login 429 при превышении, callback 429 при превышении rate,
...
**Acceptance criteria:**
- [x] `src/queue-monitor/config.js`: `AUTH_RATE_LIMIT` (bool, default `true`),
      `AUTH_RATE_LIMIT_MAX` (int, default `20`), `AUTH_RATE_LIMIT_WINDOW_MS`
      (int, default `60_000`), `AUTH_RATE_CONCURRENCY` (int, default `5`)
- [x] `createQueueMonitorConfig(environment)` возвращает эти поля
- [x] `index.js`: создаёт `rateLimiter` через `createAuthRateLimiter(config...)`
      при `authEnabled`, передаёт в `createAuthRoutes`
- [x] Если `AUTH_RATE_LIMIT=false` — `rateLimiter` не создаётся, auth-routes
      работают без guard (обратная совместимость)
- [x] `examples/bot-platform/env.example`: задокументировать новые ENV vars
- [x] `INSTALL.md`: упомянуть rate-limit vars в секции queue-monitor config

**Verification:**
- [x] `node --test tests/queue-monitor/config.test.js` — новые env vars парсятся
- [x] `node --test tests/queue-monitor/index.test.js` — wiring корректный
...
- [x] Ручная проверка: 21-й запрос к `/api/auth/login` в течение 60с → 429
- [x] Ручная проверка: 6-й параллельный callback → 429 (имитировать медленный IdP)
...
**Acceptance criteria:**
- [x] `assertSafeUrl(rawUrl)` → Promise<void> (бросает `Error` с понятным сообщением при нарушении)
- [x] Scheme check: отклоняет всё, кроме `https:` (`http:`, `file:`, `ftp:` и т.д.)
- [x] IPv4 ranges (отклоняются): `127.0.0.0/8` (loopback), `10.0.0.0/8`,
...
- [x] IPv6 ranges (отклоняются): `::1/128` (loopback), `fc00::/7` (unique-local),
      `fe80::/10` (link-local), `::ffff:0:0/96` (IPv4-mapped — проверять
      embedded IPv4 по тем же правилам)
- [x] Проверяет ВСЕ A/AAAA records (не только первый) — любой private → отказ
- [x] Ошибка/thrown message: только reason + hostname (минимизация info disclosure).
...
- [x] `options.dnsLookup` (injectable, для тестов — mock resolver)
- [x] `options.onDebug` (injectable, для логирования IP на debug-уровне)
- [x] Экспортирует `isPrivateIPv4(ip)`, `isPrivateIPv6(ip)`, `assertSafeUrl`

**Verification:**
- [x] `node --test tests/queue-monitor/auth/url-safety.test.js` — тесты проходят
- [x] Тесты покрывают: каждый IPv4 range (по репрезентативному IP),
...
**Acceptance criteria:**
- [x] `discoverEndpoints`: `await assertSafeUrl(joinUrl(issuer, DISCOVERY_PATH))`
...
- [x] `callback`: `await assertSafeUrl(ep.tokenEndpoint)` перед POST
- [x] `getUserInfo`: `await assertSafeUrl(ep.userinfoEndpoint)` перед GET
- [x] Ошибка содержит только hostname + reason, не полный URL с query/path
- [x] `createOidcClient(options)` принимает опциональные `options.dnsLookup`
      и `options.onDebug` (прокидываются в assertSafeUrl для тестов и debug-лога IP)
- [x] Существующие тесты с mock fetch (`tests/queue-monitor/auth/oidc.test.js`)
      обновлены: mock resolver возвращает `203.0.113.1` (TEST-NET, безопасный)

**Verification:**
- [x] `node --test tests/queue-monitor/auth/oidc.test.js` — тесты проходят
- [x] Новые тесты: callback бросает при private tokenEndpoint,
...
**Acceptance criteria:**
- [x] `src/queue-monitor/config.js`: `idpRequireDiscovery` (bool, default `false`)
- [x] `createOidcClient(options)` принимает `options.requireDiscovery` (bool)
- [x] При `requireDiscovery=true`: `discoverEndpoints` при `!response.ok` или
      fetch error → бросает `Error` (вместо возврата fallback-объекта)
- [x] При `requireDiscovery=false` (default): текущее поведение (fallback) —
      обратная совместимость
- [x] `index.js`: прокидывает `requireDiscovery` в `createOidcClient`
- [x] `examples/bot-platform/env.example` + `INSTALL.md`: задокументировать

**Verification:**
- [x] Тесты: `requireDiscovery=true` + 500 от IdP → ошибка (не fallback);
...
- [x] Ручная проверка: `IDP_ISSUER=https://169.254.169.254` → запуск падает с ошибкой SSRF
- [x] Ручная проверка: `IDP_REQUIRE_DISCOVERY=true` + невалидный discovery → запуск падает
