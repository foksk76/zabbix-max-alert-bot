...
**Acceptance criteria:**
- [x] `docs/brand-book.md` существует
- [x] Содержит primary palette (3-5 цветов с hex-значениями)
- [x] Содержит typographic scale (font family, sizes, weights)
- [x] Содержит tone guidelines (technical, not marketing)
- [x] Содержит usage examples (dashboard UI, docs, systemd)
- [x] Цвета экстрагированы из логотипа или обоснованы brand-логикой

**Verification:**
- [x] Файл читается, нет placeholder-текста
...
**Acceptance criteria:**
- [x] `src/tokens/colors.js` — primary, neutral, semantic (success/warning/error) palettes
- [x] `src/tokens/typography.js` — font sizes (xs/sm/base/lg/xl/2xl), line-heights, font-weights
- [x] `src/tokens/spacing.js` — 4px grid (1=4px, 2=8px, ... 16=64px)
- [x] `src/tokens/radii.js` — sm/md/lg/full
- [x] `src/tokens/shadows.js` — sm/md/lg (elevation)
- [x] `src/tokens/index.js` — единый экспорт всех tokens
- [x] Каждый модуль экспортирует и JS-объект, и CSS-переменные через `injectCSSVars()`
- [x] Primary palette совпадает с Brand Book (Task 1)
- [x] Все значения — числа (px) или строки (hex, font-family), без Tailwind-классов

**Verification:**
- [x] `cd src/queue-monitor/ui && node -e "import('./src/tokens/index.js').then(t => console.log(t.colors.primary[500]))"` → hex color
- [x] `npm test` — без регрессий
...
**Acceptance criteria:**
- [x] `tailwind.config.js` импортирует tokens
- [x] `theme.extend.colors` использует tokens.colors (primary, neutral, semantic)
- [x] `theme.extend.fontSize` использует tokens.typography.sizes
- [x] `theme.extend.spacing` расширен tokens.spacing
- [x] `theme.extend.borderRadius` использует tokens.radii
- [x] `theme.extend.boxShadow` использует tokens.shadows
- [x] Существующие Tailwind-классы в компонентах работают (backward compat)
- [x] CSS-переменные генерируются в `:root` через `index.css`

**Verification:**
- [x] `npm run build` в `ui/` — сборка проходит
- [x] Tailwind генерирует классы для token-based значений
- [x] `npm test` — без регрессий
...
- [x] Brand Book существует и содержит palette + typography
- [x] Design tokens экспортируются как JS + CSS-переменные
- [x] Tailwind build работает с tokens
- [x] `npm test` — без регрессий
- [x] Review перед Phase 2
...
**Acceptance criteria:**
- [x] `src/queue-monitor/ui/components.json` существует (shadcn config)
- [x] `src/queue-monitor/ui/src/lib/utils.js` существует (cn helper)
- [x] `src/queue-monitor/ui/src/components/ui/button.jsx` существует
- [x] `src/queue-monitor/ui/src/components/ui/card.jsx` существует
- [x] `src/queue-monitor/ui/src/components/ui/table.jsx` существует
- [x] `src/queue-monitor/ui/src/components/ui/badge.jsx` существует
- [x] `src/queue-monitor/ui/src/components/ui/input.jsx` существует
- [x] shadcn/ui компоненты кастомизированы через CSS-переменные из tokens
- [x] `tailwind.config.js` не сломан (merge с Task 3)
- [x] shadcn/ui `globals.css` импорты добавлены в `index.css`

**Verification:**
- [x] `npm run build` в `ui/` — сборка проходит
- [x] Импорт `import { Button } from './components/ui/button'` работает
- [x] `npm test` — без регрессий
...
**Acceptance criteria:**
- [x] `lucide-react` в `dependencies` в `ui/package.json`
- [x] `npm run build` — сборка проходит
- [x] Import `import { AlertCircle } from 'lucide-react'` работает
- [x] Build size не увеличивается значительно (tree-shake работает)
- [x] Нет inline SVG в компонентах (заменены на Lucide, если были)
...
- [x] shadcn/ui Button, Card, Table, Badge, Input доступны
- [x] Lucide Icons доступны
- [x] `npm run build` — сборка проходит
- [x] CSS-переменные из tokens работают с shadcn/ui
...
**Acceptance criteria:**
- [x] Импортирует `Card`, `CardHeader`, `CardContent` из `./ui/card`
- [x] Импортирует `Badge` из `./ui/badge` (для статусов)
- [x] Цвета через Tailwind-классы на основе tokens (не hardcoded hex)
- [x] Skeleton placeholder сохраняется (loading state)
- [x] Визуально идентично текущему (или лучше)
- [x] Нет inline `style={}` атрибутов

**Verification:**
- [x] `npm run build` — сборка проходит
- [x] Визуальная проверка: карточки выглядят как Card компоненты
- [x] `npm test` — без регрессий
...
**Acceptance criteria:**
- [x] Импортируют Table компоненты из `./ui/table`
- [x] Импортируют Badge из `./ui/badge`
- [x] Header/body/row/cell — shadcn/ui компоненты
- [x] Tokens для цветов (через Tailwind классы)
- [x] Toggle buttons в TopTable (source/recipient) — shadcn/ui Button
- [x] Нет inline `style={}` атрибутов

**Verification:**
- [x] `npm run build` — сборка проходит
- [x] Визуальная проверка: таблицы выглядят как Table компоненты
- [x] `npm test` — без регрессий
...
**Acceptance criteria:**
- [x] Импортирует colors из tokens
- [x] Цвета линий (delivered/failed/pending/processing) берутся из tokens
- [x] Window switcher (1h/6h/12h/24h) — shadcn/ui Button
- [x] Tooltip/axis стили — tokens
- [x] Нет hardcoded hex-цветов в компоненте

**Verification:**
- [x] `npm run build` — сборка проходит
- [x] Визуальная проверка: график использует brand-цвета
- [x] `npm test` — без регрессий
...
**Acceptance criteria:**
- [x] LoginPage: import Button from `./ui/button`
- [x] LoginPage: OAuth link styled as shadcn/ui Button (variant="default")
- [x] DashboardPage: logout button — shadcn/ui Button (variant="destructive" или "outline")
- [x] DashboardPage: refresh button — shadcn/ui Button
- [x] Layout spacing через Tailwind classes (tokens-based)
- [x] Нет inline `style={}` атрибутов
- [x] App.jsx — tokens для loading/skeleton стили

**Verification:**
- [x] `npm run build` — сборка проходит
- [x] Визуальная проверка: кнопки — shadcn/ui стили
- [x] `npm test` — без регрессий
...
- [x] Все 6 компонентов используют shadcn/ui + tokens
- [x] Нет hardcoded hex-цветов в компонентах
- [x] Нет inline `style={}` атрибутов
- [x] Визуально идентично текущему (или лучше)
- [x] `npm run build` — сборка проходит
- [x] Review перед Phase 4
...
**Acceptance criteria:**
- [x] `src/queue-monitor/ui/.storybook/main.js` существует
- [x] `src/queue-monitor/ui/.storybook/preview.js` существует
- [x] `package.json` содержит scripts: `storybook`, `build-storybook`
- [x] Storybook запускается через `npm run storybook`
- [x] `npm run build` — production build не включает Storybook
- [x] Storybook — devDependency (не в production bundle)

**Verification:**
- [x] `npm run storybook` — запускается на localhost:6006
- [x] `npm run build` — сборка проходит ( без storybook в bundle)
- [x] `npm test` — без регрессий
...
**Acceptance criteria:**
- [x] `src/stories/Button.stories.jsx` — variants (default, destructive, outline, ghost)
- [x] `src/stories/Card.stories.jsx` — basic card, card with header
- [x] `src/stories/Table.stories.jsx` — basic table, with data
- [x] `src/stories/Badge.stories.jsx` — variants (default, secondary, destructive, outline)
- [x] `src/stories/SummaryCards.stories.jsx` — with data, loading state
- [x] `src/stories/ErrorsTable.stories.jsx` — with data, empty state
- [x] Каждая story демонстрирует компонент в isolation

**Verification:**
- [x] `npm run storybook` — все stories отображаются
- [x] `npm run build` — сборка проходит
- [x] `npm test` — без регрессий
...
**Acceptance criteria:**
- [x] `docs/ui-guidelines.md` существует
- [x] Содержит: approved components (shadcn/ui + Lucide)
- [x] Содержит: forbidden patterns (inline styles, hardcoded hex)
- [x] Содержит: typical patterns (metric cards, data tables, charts)
- [x] Содержит: example — how to create a new dashboard panel
- [x] Содержит: link to Brand Book и Storybook
...
- [x] `npm test` — все тесты проходят (без регрессий)
- [x] `npm run build` в `ui/` — сборка проходит
- [x] `npm run storybook` — запускается, stories отображаются
- [x] Все компоненты используют shadcn/ui + tokens
- [x] Brand Book + AI-guidelines существуют
- [x] Нет hardcoded hex-цветов в компонентах
- [x] Нет inline `style={}` атрибутов
- [x] `docs/decisions/README.md` содержит ADR-0036
