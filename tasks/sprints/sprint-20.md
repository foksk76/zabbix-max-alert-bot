...
**Acceptance criteria:**
- [x] `MONITOR_ENABLED` (bool, default `false`) — включает queue-monitor
- [x] `MONITOR_PORT` (integer, default `9000`, range 1024–65535) — порт HTTP-сервера
- [x] `METRICS_API_KEY` (string, default `''`) — Bearer Token для `/api/metrics/*`
- [x] `MONITOR_DB_PATH` (string, default `''`) — путь к БД (пустой = инжектируется)
- [x] Функция `createQueueMonitorConfig(environment)` возвращает объект конфига
- [x] Валидация: `MONITOR_PORT` в диапазоне; `METRICS_API_KEY` не пустой
  если `MONITOR_ENABLED=true` (config-level rejection — сервер не стартует
  при отсутствии ключа, fail-fast подход)

**Verification:**
- [x] `node --test tests/queue-monitor/config.test.js` — тесты проходят
...
**Acceptance criteria:**
- [x] `createDbReader({ dbPath, logger })` — factory function
- [x] `reader.summary()` → `{ pending, processing, delivered, failed }` (GROUP BY status)
- [x] `reader.timeseries(windowSeconds)` → `[{ status, count, bucket }]` (GROUP BY status + time bucket)
- [x] `reader.topSource(limit)` → `[{ source, count }]` (GROUP BY source, ORDER BY count DESC)
- [x] `reader.topRecipient(limit)` → `[{ recipient, count }]` (парсит JSON из `payload`, GROUP BY recipient)
- [x] `reader.errors(limit)` → `[{ id, source, payload, attempts, created_at, updated_at }]` (WHERE status='failed', ORDER BY updated_at DESC)
- [x] `reader.ready()` → `boolean` (проверяет доступность БД через simple query)
- [x] `reader.close()` — закрывает readonly connection
- [x] Readonly: `better-sqlite3` с `readonly: true`, journal_mode=WAL
- [x] Все queries используют prepared statements (без string interpolation)

**Verification:**
- [x] `node --test tests/queue-monitor/db-reader.test.js` — тесты проходят
- [x] Тесты используют `:memory:` SQLite для изоляции
- [x] `npm test` — без регрессий
...
**Acceptance criteria:**
- [x] `createMonitorHttpServer(options)` — factory function
- [x] `options.port` (default 9000), `options.metricsAuth`, `options.metricsRoutes`, `options.readyzRoute`, `options.logger`
- [x] `GET /api/metrics/*` → `metricsAuth.authenticate(req)` → `metricsRoutes[endpoint](req, res)`
- [x] `GET /readyz` → `readyzRoute(req, res)`
- [x] Все остальные маршруты → 404
- [x] `start()` → Promise (resolves when listening)
- [x] `stop()` → Promise (resolves when closed)
- [x] Access log: `formatLogLine` с `MODULE_NAME = 'queue-monitor-http'`
- [x] Error handling: 500 при необработанных исключениях
- [x] CORS headers для `/api/metrics/*` (опционально, для browser-based tools)

**Verification:**
- [x] `node --test tests/queue-monitor/http-server.test.js` — тесты проходят
- [x] Тесты используют реальный `http.createServer` с порт-счетчиком
- [x] `npm test` — без регрессий
...
**Acceptance criteria:**
- [x] `createBearerAuth({ apiKey, logger })` — factory function
- [x] `auth.authenticate(req)` → `{ authenticated: true }` или `{ authenticated: false, error }`
- [x] Извлекает токен из `Authorization: Bearer xxx` header
- [x] Сравнивает с `apiKey` через `crypto.timingSafeEqual` (timing-safe)
- [x] Если `apiKey` пустой → все запросы отклоняются (503)
- [x] Логирует попытки аутентификации (успешные и неуспешные)
- [x] Не логирует сам токен (redaction)

**Verification:**
- [x] `node --test tests/queue-monitor/api-auth.test.js` — тесты проходят
- [x] Тесты проверяют timing-safe comparison
- [x] `npm test` — без регрессий
...
**Acceptance criteria:**
- [x] `createMetricsRoutes({ reader })` → `{ summary, timeseries, top, errors, discovery }`
- [x] `GET /api/metrics/summary` → `{ pending, processing, delivered, failed, total, timestamp }`
- [x] `GET /api/metrics/timeseries?window=1h` → `{ data: [{ status, count, bucket }] }`
  - Window parsing: `1h` = 3600s, `6h` = 21600s, `12h` = 43200s, `24h` = 86400s
  - Default: `1h`
- [x] `GET /api/metrics/top?by=source&limit=5` → `{ data: [{ source, count }] }` или `{ data: [{ recipient, count }] }`
  - `by` param: `source` (default) или `recipient`
  - `limit` param: 1–100, default 5
- [x] `GET /api/metrics/errors?limit=20` → `{ data: [{ id, req_id, source, payload, attempts, created_at, updated_at }] }`
  - `limit` param: 1–100, default 20
  - `req_id` включён для корреляции с audit trail (ADR-0029)
- [x] `GET /api/metrics/discovery` → Zabbix LLD format: `{ data: [{ "{#METRIC}": "delivered", "{#LABEL}": "Доставлено" }, ...] }`
- [x] Все ответы: `Content-Type: application/json`
- [x] Ошибки валидации параметров → 400 с `{ error: "..." }`
- [x] Reader errors → 500 с `{ error: "Internal server error" }`

**Verification:**
- [x] `node --test tests/queue-monitor/api-metrics.test.js` — тесты проходят
- [x] Тесты проверяют все 5 эндпоинтов + edge cases (невалидные params)
- [x] `npm test` — без регрессий
...
**Acceptance criteria:**
- [x] `createReadyzRoute({ reader })` → function(req, res)
- [x] `GET /readyz` → `200` с `{ status: "ok", db: "ready" }` если `reader.ready()` true
- [x] `GET /readyz` → `503` с `{ status: "error", db: "unavailable" }` если `reader.ready()` false
- [x] Без auth (открыт для systemd ExecStartPost)
- [x] Ответ за <100ms (readiness check не должен быть тяжелым)

**Verification:**
- [x] `node --test tests/queue-monitor/api-readyz.test.js` — тесты проходят
- [x] `npm test` — без регрессий
...
**Acceptance criteria:**
- [x] `createQueueMonitor(options)` → `{ start, stop }` (patтерн ingress)
- [x] `start()` открывает readonly DB replica, запускает HTTP-сервер
- [x] `stop()` закрывает HTTP-сервер и readonly DB connection
- [x] В `app.js`: queue-monitor добавляется в `stopHandles` после `queue-store`
- [x] Порядок остановки: worker → ingress → queue-store → queue-monitor
  (ADR-0033: reader закрывается последним, т.к. зависит от WAL writer;
  после закрытия queue-store reader теряет доступ к БД)
- [x] Если `MONITOR_ENABLED=false` — queue-monitor не создаётся
- [x] `dbPath` передаётся из config (shared с bot-platform)
- [x] Тесты shutdown ordering обновлены

**Verification:**
- [x] `node --test tests/bot-platform/app-shutdown.test.js` — тесты проходят
- [x] `npm test` — без регрессий
...
**Acceptance criteria:**
- [x] `repo-structure.test.js`: allowed set уже содержит `better-sqlite3` (проверить)
- [x] `npm test` — policy test проходит без изменений
- [x] Root `package.json` dependencies: только `better-sqlite3` + `@okta/jwt-verifier` (Sprint 20)
- [x] `openid-client` добавляется в Sprint 21, не в Sprint 20

**Verification:**
- [x] `npm test` — все 353+ тестов проходят
- [x] `node -e "console.log(Object.keys(require('./package.json').dependencies))"` — только `better-sqlite3` + `@okta/jwt-verifier`
...
- [x] `npm test` passes (353+ tests)
- [x] `MONITOR_ENABLED=true METRICS_API_KEY=test node src/bot-platform/app.js` — queue-monitor стартует на порту 9000
- [x] `GET /api/metrics/summary` returns JSON with Bearer Token
- [x] `GET /readyz` returns 200/503
- [x] Shutdown: `SIGTERM` → worker → ingress → queue-store → queue-monitor (логи)
- [x] Shutdown integration works (queue-monitor в stopHandles)
