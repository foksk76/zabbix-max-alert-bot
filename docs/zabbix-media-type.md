...
- `Severity` — необязательный: уровень важности события для выбора иконки и флага push-уведомления. Значения: `Warning` (⚠️, без push), `Average` (☢️, с push), `High` (⛔, с push), `Disaster` (🔥, с push). Если не указан или неизвестен — иконка ℹ️, без push.
- `Subject` — тема уведомления.
- `To` — идентификатор получателя в МАХ.
- `Token` — токен бота МАХ. В репозитории не хранится.
- `Trigger_status` — необязательный: статус триггера. Если `OK` — иконка ✅, push-уведомление отключено (восстановление). Если `PROBLEM` или не указан — иконка и push определяются по `Severity`.
