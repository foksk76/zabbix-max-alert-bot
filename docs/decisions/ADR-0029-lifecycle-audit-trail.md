...
Audit-события — это записи с определёнными action-именами
(`auth success`, `message queued`, `message delivered`, `message failed`).
Формат общий с trace (единый `formatLogLine`):
...
Фильтрация audit-событий: `grep 'message queued\|message delivered\|message failed\|auth success\|auth failed'`
...
Trace-события — это записи с `reqId` в module-строке
(`[module:reqId]`). `reqId` генерируется на входе HTTP-запроса
(`crypto.randomUUID()`), прокидывается через весь pipeline.
...
[info] [queue-worker:q:13] delivered {"duration_ms":45}
```

Фильтрация trace по reqId: `grep 'req:abc123'`
...
Единый формат для audit и trace:

```text
[<level>] [<module>:<reqId>] <action> {<json-context>}
```

С опциональным timestamp (передаётся через `ts` параметр):
...
Пример:
```text
[info] [jwt-source-auth:req:abc123] auth success {"sub":"zabbix","source":"zabbix"}
```

- Человек читает текст до `{`
- Logstash парсит JSON-хвост через `json` filter
- Audit-события: action = `auth success`, `message queued`, `message delivered`, `message failed`
- Trace-события: любые записи с `reqId` в module-строке

### Фильтрация

```text
# Audit-события (ключевые facts)
journalctl -u zyablik-bot-live | grep -E 'message queued|message delivered|message failed|auth success|auth failed'

# Trace по конкретному reqId
journalctl -u zyablik-bot-live | grep 'req:abc123'

# Все входящие запросы
journalctl -u zyablik-bot-live | grep 'ingress'
```
...
filter {
  grok {
    match => { "message" => "\[%{LOGLEVEL:level}\] \[%{DATA:module}:%{DATA:reqId}\] %{WORD:action} %{GREEDYDATA:json_context}" }
  }
  if [json_context] =~ /^\{/ {
...
- Audit trail — для быстрого просмотра и алертинга (grep по action-именам: `message queued`, `delivered`, `failed`)
- Trace — для глубокого разбора инцидента (grep по `reqId`)
...
Минус: слишком шумно для быстрого просмотра. Оператору нужен grep по action-именам (`message queued`, `delivered`), а не grep по reqId для каждого события. Отклонено.
...
| `core/logger.js` | `formatLogLine()` helper для unified формата |
...
### Поправка от 2026-07-23: единый формат

Первоначально ADR описывал два отдельных формата:
`[audit] action params` и `[trace:req:<id>] action params`.
При реализации использован единый формат `formatLogLine`:
`[level] [module:reqId] action {json}`.

Audit-события отличаются action-именами (`message queued`,
`message delivered`, `message failed`, `auth success`, `auth failed`).
Trace-события определяются наличием `reqId` в module-строке.

Фильтрация audit: `grep -E 'message queued|message delivered|message failed|auth success|auth failed'`.
Фильтрация trace по reqId: `grep 'req:abc123'`.
